...
AI-powered blood work analysis for [NanoClaw](https://nanoclaw.dev). Parse lab reports, calculate biological age, and get evidence-based health recommendations — all locally on your machine.
...
Once files are synced, just ask Claude to include your HealthKit data. It will scan the iCloud Drive folder automatically.
...
Once saved to the reports directory, ask Claude to include HealthKit data in your next analysis.
...
4. Ask Claude to analyze your health data — it will read the exported files automatically
...
Then ask Claude to analyze it. Claude will find the file, read the PDF, extract biomarkers, and run the full analysis pipeline.
...
```bash
cd ~/.claude/skills/otto-lab/scripts
